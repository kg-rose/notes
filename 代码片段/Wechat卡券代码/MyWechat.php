...
class MyWechat extends Component {
    ...
    CONST EVENT_CARD_USER_CONSUME = 'USER_CONSUME_CARD';        //卡券 - 用户核销卡券
    ...
    const USER_INFO_ACCESSTOKE_URL = '/sns/userinfo?';
    ...
    const JSAPI_URL = 'https://api.weixin.qq.com/cgi-bin/ticket/getticket?';
    ...
    const BATCH_TAGIGING = '/tags/members/batchtagging?';
    const GET_TAGGING ='/tags/get?';
    ///多客服相关地址
    // const CUSTOM_SERVICE_GET_RECORD = '/customservice/getrecord?';
    const CUSTOM_SERVICE_GET_RECORD = '/customservice/msgrecord/getmsglist?';
...
///微信摇一摇周边
    ...
    private $access_token_refresh;

    const CACHE_ACC_NAME = 'access_token';
    const CACHE_JS_NAME = 'jsapi_ticket';
    const CACHE_JS_CARD_NAME = 'jsapi_ticket_for_card';
    ...
    public function valid($return = false) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
                ...
                Yii::trace($this->encodingAesKey, 'wechat_app');
                $pc = new Prpcrypt($this->encodingAesKey);
                Yii::trace($encryptStr, 'wechat_app');
                Yii::trace($this->appid, 'wechat_app');
    ...
    private function log($log) {
        if ($this->debug) {
            $file_path = 'log_reg/' . date('Ymd');
            Fuc::makeDir($file_path);
            $file = $file_path . '/wechat_log.txt';
            error_log("error:" . date('Y/m/d H:i:s') . $log . "\r\n", 3, $file);
        }
    }
    ...
    private function error($data) {
        ...
        $this->removeCache(self::CACHE_ACC_NAME);

        Yii::info($data, 'wechatUserInfo');

        $this->log('WEIXIN API errcode:[' . $this->errCode . '] errmsg:[' . $this->errMsg . ']' . date('Y-m-d H:i:s'));
    }
    ...
    public function getRevCardConsume() {
        if (isset($this->_receive['ConsumeSource']))     //卡券 ID
            $array['ConsumeSource'] = $this->_receive['ConsumeSource'];
        if (isset($this->_receive['LocationName']) && !empty($this->_receive['LocationName']))    //门店名称，当前卡券核销的门店名称（只有通过自助核销和买单核销时才会出现该字段）
            $array['LocationName'] = $this->_receive['LocationName'];
        if (isset($this->_receive['StaffOpenId']) && !empty($this->_receive['StaffOpenId'])) //核销该卡券核销员的openid（只有通过卡券商户助手核销时才会出现）
            $array['StaffOpenId'] = $this->_receive['StaffOpenId'];
        if (isset($array) && count($array) > 0) {
            return $array;
        } else {
            return false;
    ...
    public function getMaterailBatch($type, $offset = 0, $count = 20) {
    ...
    public function getMaterial($mediaId){
        ...
        if ($result) {
            ...
            $filename = 'media_'.$mediaId.'.jpg';
            $filepath = "/uploads/picture/".$filename;
            $local_file = fopen(Yii::getAlias('@webroot').$filepath, 'w');
            if (false !== $local_file){
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function get_token() {
        $wechat_info = WechatInfo::findOne(1);
        if (microtime(true) >= $wechat_info->expiretime)
            return $this->checkAuth($this->appid, $this->appsecret);
        else
            return $wechat_info->token;
    }
    ...
    protected function setCache($cachename, $value, $expired) {
        $expired === null && $expired = $this->cacheTime;
        return \Yii::$app->cache->set($this->getCacheKey($cachename), $value, $expired);
    }
     ...
     * @return mixed
     */
    protected function getCache($cachename, $defaultValue = null) {

        return \Yii::$app->cache->get($this->getCacheKey($cachename), $defaultValue);
    }
    ...
    protected function removeCache($cachename) {
        ...
        return \Yii::$app->cache->delete($this->getCacheKey($cachename));
    }
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '') {
        ...
        if ($rs = $this->getCache(self::CACHE_ACC_NAME)) {
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 3000 : 1200;
            $expire = 1200;
            $this->setCache(self::CACHE_ACC_NAME, $this->access_token, $expire);
            return $this->access_token;
        }
        return false;
    }
...
//        print_r($result);
     ...
     */
    public function uploadMaterial($data, $type) {

        $data = Yii::getAlias('@webroot') . $data;
        if (!$this->access_token && !$this->checkAuth())
            return false;
        ...
        if (class_exists('\CURLFile')) {//关键是判断curlfile,官网推荐php5.5或更高的版本使用curlfile来实例文件
            $post_data['media'] = new \CURLFile(realpath($data), 'image/jpeg');
        } else {
            $post_data ['media'] = '@' . $data;
        }
        $result = $this->http_post(self::API_URL_PREFIX . self::ADD_MATERIAL . 'access_token=' . $this->access_token, $post_data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->error(($json));
     ...
     * @return boolean|array
     */
    public function uploadThumbMedia($url, $type) {

        $data = Yii::getAlias('@webroot') . $url;
        if (!$this->access_token && !$this->checkAuth())
            return false;
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_UPLOAD_URL . 'access_token=' . $this->access_token . "&type=" .$type , $post_data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getQRCodeForName($scene_id, $type = 0, $expire = 1800) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
            'action_name' => $type ? "QR_LIMIT_SCENE" : "QR_SCENE",
    ...
    public function getUserInfoByAccesstoken($openid, $access_token) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_URL_PREFIX . self::USER_INFO_ACCESSTOKE_URL . 'access_token=' . $access_token . '&openid=' . $openid);


        Yii::info($result, 'wechatUserInfo');
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 删除分组 【删除一个用户分组，删除分组后，所有该分组内的用户自动进入默认分组】
     * @param string $name 分组名称
     * @return boolean|array
     */
     ...
     */
    public function batchMembersTagging($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;

        $result = $this->http_post(self::API_URL_PREFIX . self::BATCH_TAGIGING . 'access_token=' . $this->access_token, self::json_encode($data));
    ...
    public function getTagging() {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        echo  self::API_URL_PREFIX . self::GET_TAGGING . 'access_token=' . $this->access_token;

        $result = $this->http_get(self::API_URL_PREFIX . self::GET_TAGGING . 'access_token=' . $this->access_token);
     ...
     * @return string
     */
    public function getOauthRedirect($callback, $state = '', $scope = '1') {
    ...
    public function getJsApiTicket() {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if ($rs = $this->getCache(self::CACHE_JS_NAME)) {

            $this->jsapi_ticket = $rs;
            return $rs;
        }

        $result = $this->http_get(self::JSAPI_URL . 'access_token=' . $this->access_token . '&type=jsapi');
        ...
        if ($result) {
            ...
            $this->setCache(self::CACHE_JS_NAME, $this->jsapi_ticket, $expire);
    ...
    public function getCustomServiceMessage($data) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CUSTOM_SERVICE_GET_RECORD . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     *
     */
    public function checkCardCode($code, $card_id = '', $check_consume = false) {
        $data = array(
            'code' => $code,
            'card_id' => $card_id,
